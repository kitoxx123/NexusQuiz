...
// Admin panelinden gelen form verilerini okumak için gerekli ayarlar
app.use(express.json());
app.use(express.urlencoded({ extended: true }));
app.use(express.static(path.join(__dirname, 'public')));

// CANLI AYARLAR DEPOSU
let siteAyarlari = {
    isim: "NexusQuiz",
    motif: "grid",
    renk: "#0b0c10"
...
// Admin Panelini Açma Yolu
app.get('/admin', adminSecured, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'admin.html'));
});

// KÖPRÜ 1: Dışarıya Mevcut Ayarları Paylaşan Yol
app.get('/api/ayarlar', (req, res) => {
    res.json(siteAyarlari);
});

// KÖPRÜ 2: Panelden Gelen Görünüm Ayarlarını Kaydeden Yol
app.post('/api/ayarlar', adminSecured, (req, res) => {
    const { isim, motif, renk } = req.body;
    if (isim) siteAyarlari.isim = isim;
    if (motif) siteAyarlari.motif = motif;
    if (renk) siteAyarlari.renk = renk;
    res.json({ status: "success", message: "Görünüm güncellendi!" });
});

// KÖPRÜ 3: Panelden Gelen Soruları Listeye Ekleyen Yol
app.post('/api/soru-ekle', adminSecured, (req, res) => {
    const { kategori, text, options, correct } = req.body;
    
    if (!testPaketleri[kategori]) {
        testPaketleri[kategori] = [];
    }
    
    testPaketleri[kategori].push({
        text: text,
        options: options,
        correct: parseInt(correct)
    });
    
    res.json({ status: "success", message: "Soru başarıyla havuza eklendi!" });
});
